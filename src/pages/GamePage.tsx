// ЗМІНИ:
// - Видалено useState для inLibrary та checkingLibrary
// - Видалено useEffect, який робив окремий запит для бібліотеки
// - inLibrary тепер обчислюється прямо з library (React.Context)
// - Видалено console.log
import { useEffect, useState } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { useWishlist } from "../components/WishlistContext";
import { useCart } from "../components/CartContext";
import { useLibrary, LibraryItem } from "../components/LibraryContext";
import {
  Star,
  ArrowLeft,
  Edit2,
...
function GamePage() {
  ...
  const { addToWishlist, removeFromWishlist, isGameWishlisted } =
    useWishlist();
  ...
  useEffect(() => {
    ...
    if (/^\d+$/.test(id)) {
      setError(
        t("invalid_game_id") +
          ". Please remove this game from wishlist/library and add again."
      );
      return;
    }
  ...
  useEffect(() => {
    if (!game) return;
    const rawDealID = game?.dealID
  || (game?.gameID != null ? String(game.gameID) : undefined)
  || game?.gameInfo?.dealID
  || id;
  const dealID = rawDealID ? (rawDealID.includes('%') ? decodeURIComponent(rawDealID) : rawDealID) : undefined;
    ...
    setIsInCart(
      cartItems.some(
        (item) => item.gameID === dealID || item.title === game.gameInfo?.name
      )
    );
  }, [game, isGameWishlisted, cartItems, id]);
  ...
  const dealID = game?.dealID || game?.gameInfo?.dealID || id;
  const inLibrary = dealID
  ? library.some((item: LibraryItem) => item.dealID === dealID)
  : false;

  console.log('[DEBUG] GamePage:', game?.gameInfo?.name, 'dealID:', dealID, 'inLibrary:', inLibrary, 'library dealIDs:', library.map(i => i.dealID));

  const handleWishlistToggle = () => {
    if (!dealID || !game?.gameInfo?.name) return;
    if (isWishlisted) {
  ...
  const handleCartToggle = () => {
    ...
    console.log('[DEBUG] GamePage handleCartToggle, dealID:', dealID);
  };

  const handleSubmitReview = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentUser || !userProfile) {
      setFormError(t("login_to_review"));
      return;
    }
    if (!id || id.includes("/")) return;
    ...
    if (comment.length > 1000) {
      setFormError(t("error_comment_too_long"));
      return;
    }
  ...
  const fetchComments = async (reviewId: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/reviews/${reviewId}/comments`
      );
      const data = await res.json();
  ...
  const handleDeleteComment = async (commentId: string, reviewId: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/reviews/comments/${commentId}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId: currentUser.uid }),
  ...
  const locale =
    i18n.language === "en"
      ...
      : "uk-UA";
  const goBack = () =>
    window.history.length > 2 ? navigate(-1) : navigate("/store");
  ...
  if (error)
    return (
      <div className="min-h-screen bg-black mt-[160px] font-mono text-white flex flex-col items-center justify-center">
        ...
        <button
          onClick={goBack}
          className="inline-flex items-center gap-2 bg-neutral-900/50 border border-lime-500/30 text-lime-400 px-4 py-2 rounded-sm hover:bg-lime-500/20"
        >
          ← {t("back")}
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-black mt-[160px] font-mono text-white py-12 px-4">
      <div className="max-w-4xl mx-auto mt-16 relative">
        <button
          onClick={goBack}
          className="inline-flex items-center gap-2 bg-neutral-900/50 border border-lime-500/30 text-lime-400 px-4 py-2 rounded-sm hover:bg-lime-500/20 mb-6"
        >
          ← {t("back")}
        </button>
        <div className="border border-lime-500/30 rounded-lg p-6 bg-neutral-950/95">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              {inLibrary ? (
                <div className="flex items-center justify-center gap-3 py-3 rounded border-2 border-green-500/50 bg-green-500/10 text-green-400 font-semibold text-lg animate-pulse">
                ...
                <>
                  <button
                    ...
                    className={`w-full py-3 rounded border flex items-center justify-center gap-2 ${
                      isWishlisted
                        ? "border-lime-400 bg-lime-500/20"
                        : "border-lime-500 bg-lime-500/10"
                    } text-lime-400 hover:bg-lime-500 hover:text-black transition`}
                  >
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            <div className="border border-lime-500/20 rounded p-4 text-center">
              <div className="text-lime-400 uppercase text-sm mb-1">
                {t("steam_rating")}
              </div>
              <div className="text-2xl font-bold">
                {game.gameInfo?.steamRatingText || "N/A"}
              </div>
            </div>
            <div className="border border-lime-500/20 rounded p-4 text-center">
              <div className="text-lime-400 uppercase text-sm mb-1">
          ...
          <div className="mt-12">
            ...
            {currentUser ? (
              <form
                ...
                className="bg-neutral-900/50 border border-lime-500/30 rounded p-4 mb-6 space-y-3"
              >
                <div className="flex items-center gap-3">
                  <img
                    src={
                      userProfile?.avatar || "../src/assets/avatar.png"
                    }
                    alt=""
                    className="size-8 rounded-full border border-lime-500/30"
                  />
                ...
                <div className="flex gap-2">
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex-1 bg-lime-500/10 border border-lime-500 text-lime-400 py-2 rounded hover:bg-lime-500 hover:text-black transition"
                  >
                    {editingReviewId
                      ? t("update_review")
                      : t("submit_review")}
                  ...
                  {editingReviewId && (
                    <button
                      ...
                      className="px-3 py-2 bg-neutral-900 border border-lime-500/30 text-lime-400 rounded hover:bg-neutral-800"
                    >
                      {t("cancel")}
                    </button>
                  )}
            ...
            {reviewsError && (
              <p className="text-red-500 text-center mb-4">{reviewsError}</p>
            )}
            ...
            {reviews.map((review) => (
              <div
                key={review.id}
                className="border border-lime-500/20 rounded p-3 mb-2"
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <img
                      src={
                        review.userAvatar || "../src/assets/avatar.png"
                      }
                      alt=""
                      className="size-6 rounded-full border border-lime-500/30"
                    />
                ...
                <p className="text-neutral-300 text-sm mt-2 whitespace-pre-line">
                  {review.comment}
                </p>
                      ...
                      {loadingComments[review.id!] ? (
                        ...
                        <>
                          ...
                          {commentsMap[review.id!]?.map((comment) => (
                            <div
                              key={comment.id}
                              className="flex gap-2 items-start pl-2 border-l-2 border-lime-500/20"
                            >
                              <img
                                src={
                                  comment.userAvatar ||
                          ...
                          {currentUser && (
                            <div className="flex items-center gap-2 mt-2">
                              ...
                              <span className="text-xs text-neutral-600">
                                {(commentInputs[review.id!] || "").length}/500
                              </span>
                              <button
                                onClick={() => handleAddComment(review.id!)}
                                disabled={
                                  !commentInputs[review.id!]?.trim()
                                }
